<template>
  <div class="container">
    <form>
      <div v-for="(formItem, formItemNum) in formGroup" :key="formItemNum">
        <label :for="formItem.for">{{ formItem.title }}</label>
        <input type="text" :id="formItem.id" :placeholder="formItem.placeholder" v-model="customerInfo[formItem.id]">
      </div>
      <div>
        <label for="message">備註 :</label>
        <textarea rows="5" cols="50" id="message" v-model="customerInfo.message"></textarea>
      </div>
    </form>
    <div class="submit">
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const changePage = (btn) => {
      switch (btn.title) {
        case '上一步':
          emit('changeComponent', 'CheckoutShoppingCartContent');
          break;
        case '送出訂單':
          break;
      }
    }
    console.log(74, customerInfo)
    return {
      formGroup,
      customerInfo,
      submitBtn,
